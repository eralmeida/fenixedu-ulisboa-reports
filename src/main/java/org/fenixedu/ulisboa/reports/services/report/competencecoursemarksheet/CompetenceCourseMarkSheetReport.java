...
public class CompetenceCourseMarkSheetReport implements Comparable<CompetenceCourseMarkSheetReport> {
    ...
    public int compareTo(final CompetenceCourseMarkSheetReport o) {
        ...
        final Comparator<CompetenceCourseMarkSheetReport> byCourseName = (x, y) -> instance.compare(
                x.getComptenceCourseName(), y.getComptenceCourseName());
        final Comparator<CompetenceCourseMarkSheetReport> byCode = (x, y) -> instance.compare(
                x.getCompetenceCourseCode(), y.getCompetenceCourseCode());
        final Comparator<CompetenceCourseMarkSheetReport> byCreationDate = Comparator.comparing(
                x -> x.getCompetenceCourseMarkSheet().getCreationDate());
        final Comparator<CompetenceCourseMarkSheetReport> byCheckSum = (x, y) -> instance.compare(x.getCheckSum(),
                y.getCheckSum());

        return byCourseName.thenComparing(byCode).thenComparing(byCreationDate).thenComparing(byCheckSum).compare(this,
                o);
    }
    ...
    public String getEvaluationSeason() {
        return EvaluationSeasonServices.getDescriptionI18N(competenceCourseMarkSheet.getEvaluationSeason())
                .getContent();
    }
    ...
    public String getLastSubmissionDate() {
        final CompetenceCourseMarkSheetStateChange stateChange = competenceCourseMarkSheet.getStateChangeSet()
                .stream()
                .filter(sc -> sc.isSubmitted())
                .sorted()
                .findFirst()
                .orElse(null);
        return stateChange == null ?
               bundle("CompetenceCourseMarkSheetReport.submissionDate.notfound") : DateTimeFormat
                       .forPattern("yyyy-MM-dd HH:mm:ss")
                       .print(stateChange.getDate());
    }
    ...
    public String getLastState() {
        return competenceCourseMarkSheet.getStateChangeSet().stream().sorted().map(
                sc -> sc.getState().getDescriptionI18N().getContent()).findFirst().orElse(null);
    }
