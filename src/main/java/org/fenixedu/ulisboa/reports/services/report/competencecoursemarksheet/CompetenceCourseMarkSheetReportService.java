...
public class CompetenceCourseMarkSheetReportService {
    ...
    private Collection<CompetenceCourseMarkSheetReport> process(final ExecutionSemester executionSemester) {
        return buildSearchUniverse(executionSemester).stream().map(this::buildReport).collect(Collectors.toSet());
    }
